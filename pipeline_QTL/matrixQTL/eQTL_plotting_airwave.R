...
########################
# Rename SNP if problematic:
# Keep old name for searching though:
snp_old <- snp # eg snp_old="22:18906839"
snp_old

if (grepl('[:]', as.character(snp))) {
  snp <- sprintf('x%s', gsub(":", "_", as.character(snp)))
} else {
  snp <- snp
}
snp
...
names(snp_probe_data)[1] <- snp # rename if SNP name was eg 22:18906839 and now x22_18906839
names(snp_probe_data)
sapply(snp_probe_data, class)
...
names(snp_probe_PC_data)[1] <- snp # rename if SNP name was eg 22:18906839 and now x22_18906839
sapply(snp_probe_PC_data, class)
# pass_formula <- as.formula(sprintf('%s ~ %s + .', probe, snp))
...
eQTL_data[which(eQTL_data[, SNP] == snp_old), ]
...
ggplot(snp_probe_data, aes(as.factor(snp_probe_data[, snp]), snp_probe_data[, probe])) + 
  ...
  ylab('Average metabolite (datapoint, uncorrected) abundance levels') + 
  xlab('Genotype') + ggtitle(plot_title) + theme_minimal() +
  theme(text = element_text(size = 8))
file_name <- sprintf('uncorrected_%s_%s_%s.pdf', snp, probe, plot_file_name)
file_name
...
ggplot(gex_corrected_snp_df, aes(genotype, expression)) +
  ...
  xlab('Genotype') + ggtitle(plot_title) + theme_minimal() +
  theme(text = element_text(size = 8))
file_name <- sprintf('corrected_%s_%s_%s.pdf', snp, probe, plot_file_name)
file_name
ggsave(plot = last_plot(), filename = file_name)
