...
Bind two data frames. This is specifically for principal component files derived from
genotype and phenotype data. This is a simple script with rbind, no checks are done. 

Usage and options
=================

To run, type:
Rscript rbind_dataframes.R --file1 <FILE> --file2 <FILE> [options]

Usage: rbind_dataframes.R (--file1 <FILE>) (--file2 <FILE>) [options]
       rbind_dataframes.R [options]
...
--file1-PCs <int>             Number of PCs from file 1 to keep. [default: 10]
--file2-PCs <int>             Number of PCs from file 1 to keep. [default: 30]
...
Columns must already match, this is a simple rbind script.
...
# rbind into one file, choice of PCs is arbitrary:
input_data_1 <- as.data.frame(input_data_1)
input_data_2 <- as.data.frame(input_data_2)
all_covar_PCs <- rbind(input_data_1[1:file1_PCs, ], input_data_2[1:file2_PCs, ])
# all_covar_PCs[1:5, 1:5]
print('Shape of merged file:')
dim(all_covar_PCs)
